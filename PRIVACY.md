...
Asked to play something, MacB fetches the YouTube results page for the words it was given and reads exactly one thing out of it: the first video identifier, eleven characters of a fixed alphabet. No title, description, channel or comment is taken from the page, so nothing anybody has written on YouTube can reach the model or act as an instruction. For Spotify and Music it only opens their own search. Putting the Mac to sleep, sleeping the display and locking the screen are asked about every time; MacB cannot shut down or restart, and never types or handles a password.
