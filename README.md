...
- The MacB AI tab keeps local watcher cards for product prices, website text changes, GitHub release tags and system thresholds. You can create them by talking to MacB AI; the manual form is only a fallback. Checks run on a quiet schedule, store only the last value/baseline locally, and surface changes in the notch.
