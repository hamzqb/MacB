...
@MainActor final class ClipboardShelfStore: ObservableObject {
    ...
    func refresh() {
        ...
        guard !privateTypes.contains(where: { pasteboard.availableType(from: [NSPasteboard.PasteboardType($0)]) != nil }) else { return }
        // A key or token copied from a web page carries no concealed mark;
        // it is recognised by its shape and never kept.
        if let text = pasteboard.string(forType: .string), SecretDetector.looksLikeSecret(text) { return }
    ...
    private func restoreHistory() {
        ...
        // A credential kept by an older MacB, before keys were recognised,
        // does not come back, and the file is rewritten without it.
        let safe = stored.filter { entry in !SecretDetector.looksLikeSecret(entry.text) }
        let fresh = safe.filter { entry in !items.contains(where: { equivalent($0, entry) }) }
        items.append(contentsOf: fresh)
        trimHistory()
        if safe.count != stored.count { saveHistoryIfKept() }
    }
